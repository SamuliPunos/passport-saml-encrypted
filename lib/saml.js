...
SAML.prototype.getElement = function (parentElement, elementName) {
  if (parentElement['saml2:' + elementName]) {
    return parentElement['saml2:' + elementName];
  } else if (parentElement['samlp:' + elementName]) {
    return parentElement['samlp:' + elementName];
  } else if (parentElement['saml:' + elementName]) {
...
SAML.prototype.validateResponse = function (samlResponse, callback) {
  ...
  if (self.options.encryptedSAML && self.options.privateCert && encryptedDataNode) {
    ...
    else //trim the unwanted characters after closing </saml2:Assertion>
    {
      var nIndex = samlAssertion.indexOf("</saml2:Assertion>");
      if (nIndex != -1) {
        var validStringLen = nIndex + ("</saml2:Assertion>".length);
        samlAssertion = samlAssertion.slice(0, validStringLen);
      } else {
        nIndex = samlAssertion.indexOf("</saml:Assertion>");
        validStringLen = nIndex + ("</saml:Assertion>".length);
        samlAssertion = samlAssertion.slice(0, validStringLen);
      }
  ...
  {
    var samlAssertionNode = xmlCrypto.xpath(xmlDomDoc, "//*[local-name(.)='Assertion']")[0];
    ...
    if (assertion) {
      ...
      if (attributes) {
        attributes.forEach(function (attribute) {
          ...
          profile[attribute['$'].FriendlyName ? attribute['$'].FriendlyName : attribute['$'].Name] = (textValues.length === 1) ? textValues[0] : textValues;
        });
      }
